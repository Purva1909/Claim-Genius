...
import type {Student} from '../type/type'
...
const updateStudent = async (First_Name:string, Last_Name:string, DOB:string, Mobile_No:string, Address:string,studentId:number):Promise<void>=>{
...
const total = async (fetchStudent:string):Promise<number> =>{
    const [totalPage]= await db.query(`SELECT COUNT(ID) as count FROM registration_form WHERE First_Name like '%${fetchStudent || ""}%' OR Last_Name like '%${fetchStudent || ""}%' OR DOB like '%${fetchStudent || ""}%' OR Mobile_No like '%${fetchStudent || ""}%' OR Address like '%${fetchStudent || ""}%'`);
...
export default  {insertStudent, delStudent, updateStudent, pagination, total, checkId}