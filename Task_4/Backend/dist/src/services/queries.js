...
const total = (fetchStudent) => __awaiter(void 0, void 0, void 0, function* () {
    const [totalPage] = yield db_1.default.query(`SELECT COUNT(*) as count FROM registration_form WHERE First_Name like '%${fetchStudent || ""}%' OR Last_Name like '%${fetchStudent || ""}%' OR DOB like '%${fetchStudent || ""}%' OR Mobile_No like '%${fetchStudent || ""}%' OR Address like '%${fetchStudent || ""}%'`);
    return (totalPage);
});
