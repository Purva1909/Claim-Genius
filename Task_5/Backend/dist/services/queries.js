...
const updateStudent = (First_Name, Last_Name, DOB, Mobile_No, Address, myFile, studentId) => __awaiter(void 0, void 0, void 0, function* () {
    yield db_1.default.query(`UPDATE registration_form SET First_Name=?, Last_Name=?, DOB=?, Mobile_No=?, Address=?, myFile=? WHERE ID=?`, [First_Name, Last_Name, DOB, Mobile_No, Address, myFile, studentId]);
});
...
const total = (fetchStudent) => __awaiter(void 0, void 0, void 0, function* () {
    const [totalPage] = yield db_1.default.query(`SELECT COUNT(ID) as count FROM registration_form WHERE First_Name like '%${fetchStudent || ""}%' OR Last_Name like '%${fetchStudent || ""}%' OR DOB like '%${fetchStudent || ""}%' OR Mobile_No like '%${fetchStudent || ""}%' OR Address like '%${fetchStudent || ""}%' OR myFile like '%${fetchStudent}%'`);
...
const documentuploaded = (userID, file) => __awaiter(void 0, void 0, void 0, function* () {
    const data = yield db_1.default.query('INSERT INTO file_upload (userID,file) VALUES (?,?)', [userID, file]);
    return data;
});
const fetchImage = (userID) => __awaiter(void 0, void 0, void 0, function* () {
    const [data] = yield db_1.default.query(`SELECT file from file_upload where userID = ? `, [userID]);
    return data;
});
const updateImage = (userID, file) => __awaiter(void 0, void 0, void 0, function* () {
    const [data] = yield db_1.default.query('UPDATE user_table SET file = ? WHERE userID =?', [file, userID]);
});
exports.default = { insertStudent, delStudent, updateStudent, pagination, total, checkId, documentuploaded, fetchImage, updateImage };
